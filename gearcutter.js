...
EOF`;
  ...
  var DxfRecorder = class {
    ...
    getDxfText() {
      const template = DXF_TEMPLATE.split("\n");
      const replacers = {
        ...
        "@@UUIDGEN": (dest) => {
          dest.push(`{${v4_default().toUpperCase()}}`);
        }
      };
      const output = [];
    ...
    getJulianCreateDay() {
      return String(this.createDate.getTime() / 864e5 + 24405875e-1);
    }
    drawCircle(drawProps, cx, cy, radius) {
  ...
  var DxfPen = class {
    constructor(recorder, drawProps) {
      this.havePoint = false;
      this.havePath = false;
      this.lastx = 0;
      this.lasty = 0;
  ...
  var REPLACER_PATTERN = /^ *@@/;
  function templateReplace(replacers, dest, src) {
...
0.0`.split("\n").map((s) => s.replace("\r", ""));
  var DxfLine = class {
    constructor(recorder, x0, y0, x1, y1) {
      this.replacers = {
        "@@x0": String(x0),
        "@@y0": String(y0),
        "@@x1": String(x1),
...
@@y2
 30
0.0`.split("\n").map((s) => s.replace("\r", ""));
  var DxfQuadSpline = class {
    constructor(recorder, x0, y0, x1, y1, x2, y2, w) {
      this.replacers = {
        "@@x0": String(x0),
        "@@y0": String(y0),
        "@@x1": String(x1),
  ...
  // src/LastPointCapturePen.ts
  var LastPointCapturePen = class {
    moveTo(x, y) {
      this.x = x;
      this.y = y;
    }
  ...
  function update() {
    ...
    document.getElementById("link1").download = `pinion${pinionTeeth}pa${paStr}.svg`;
    document.getElementById("svgTitle1").innerText = `Pinion (${pinionSegsPerTooth} arcs/tooth)`;
    document.getElementById("pinionDxfButton").onclick = () => downLoadDxf(`pinion${pinionTeeth}pa${paStr}.dxf`, sizeUnit, dxfScale, pinionPath, pinionRadius, pinionTeeth);
    if (gearSvgUrl) {
    ...
    document.getElementById("link2").download = `gear${gearTeeth}pa${paStr}.svg`;
    document.getElementById("svgTitle2").innerText = `Gear (${gearSegsPerTooth} arcs/tooth)`;
    document.getElementById("gearDxfButton").onclick = () => downLoadDxf(`gear${gearTeeth}pa${paStr}.dxf`, sizeUnit, dxfScale, gearPath, gearRadius, gearTeeth);
  }
  function submit() {
    const fields = [
  ...
  function downLoadDxf(name, units, scale, path, radius, teeth) {
    if (tempObjectUrl) {
  ...
  function createDxfObject(units, scale, path, pitchRadius, nTeeth) {
