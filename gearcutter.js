...
EOF`;
  ...
  var DxfRecorder = class {
    ...
    drawCircle(_drawProps, cx, cy, radius) {
...
0.0
 70
@@closedflag`.split("\n").map((s) => s.replace("\r", ""));
  var POLYLINE_END_TEMPLATE = `  0
SEQEND`.split("\n").map((s) => s.replace("\r", ""));
  var POLYLINE_VERTEX_TEMPLATE = `  0
VERTEX
  8
0
 10
...
0.0
 42
@@bulge`.split("\n").map((s) => s.replace("\r", ""));
  var DxfPolyLine = class {
    constructor(recorder, startx, starty) {
      this.recorder = recorder;
      this.replacers = {
        "@@closedflag": "0"
      };
